...
void insert(char *name, char *title, int num)
{
  ...
  mp3->title = (char *) malloc(strlen(title) + 1);  // malloc space for name
  strcpy(mp3->name, name);                        // "assign" name via copy
  strcpy(mp3->title, title);                        // "assign" name via copy
  mp3->data = num;                                // assign data value
  mp3->next = NULL;
  mp3->prev=NULL;                                 //* initially set to NULL until the temp node is set
  ...
  else
  {
    ...
    mp3->prev=temp;  //* assign the previous mp3 
  }
}
